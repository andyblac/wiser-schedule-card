...
class WiserSchedulesPanel extends HTMLElement {
  ...
  _cardConfig(hub) {
    return {
      ...
      type: "custom:wiser-schedule-card", hub,
    };
  }
  ...
  async _saveEditor() {
    ...
    try {
      await this._hass.callWS({
        type: "wiser/schedules_panel/configure", configs: this._drafts,
      });
  ...
  async _loadCards() {
    ...
    try {
      ...
      if (Card?.panelApiVersion !== 1) {
        throw new Error("Wiser Schedules needs its matching schedule card build. Update the card resource and refresh the browser.");
      }
